...
// Accepter une demande d'un manager
acceptManagerRequest(projectId: number, managerId: number): Observable<IApiResponse> {
  ...
  return this._http.put<IApiResponse>(
    `${baseUrl}/admin/attributeManagerToProject?projectId=${projectId}&managerId=${managerId}`,null).pipe(catchError(this.handleError));
}
