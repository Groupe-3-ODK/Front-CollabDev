...
export class TaskService {
  ...
  // AJOUT dans TaskService
createTask(taskData: { title: string; projectId: number }) : Observable<any> {
  return this._http.post<any>(`${environment.API_BASE_URL}/tasks`, taskData);
}

assignTask(taskId: number, memberId: number) {
  return this._http.post<TaskService>(`${environment.API_BASE_URL}/tasks/${taskId}/assign/${memberId}`, {});
}
